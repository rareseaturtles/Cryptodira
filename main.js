...
function debounce UpdateTokenInfo() {
...
function displayCachedTokenData() {
 ...
 if (cachedData) {
 const { price, liquidity, marketCap, timestamp } = JSON.parse(cachedData);
 const now = Date.now();
 const cacheAge = now - timestamp;
 const maxCacheAge = 24 * 60 * 60 * 1000; // 24 hours
 if (cacheAge < maxCacheAge) {
 console.log('Displaying cached token data:', { price, liquidity, marketCap });
...
async function updateTokenInfo() {
 ...
 const tokenData = { price, liquidity, marketCap, timestamp: Date.now() };
 localStorage.setItem('tokenData', JSON.stringify(tokenData));
 const fetchTime = performance.now() - startTime;
 console.log(`Updated token data cache in ${fetchTime.toFixed(2)}ms:`, tokenData);
...
window.addEventListener('load', () => {
 displayCachedTokenData();
 setTimeout(() => updateTokenInfo(), 180000); // Initial fetch after 3 minutes
});